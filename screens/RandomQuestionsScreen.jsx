...
export default function RandomQuestionsScreen() {
    ...
    const validateAnswer = () => {
        const allDesiredAreTrue = question.Answer.every(answer => checkedAnswers[answer] === true);
      
        const otherKeys = Object.keys(checkedAnswers).filter(key => !question.Answer.includes(key));
      
        const allOthersAreFalseOrUndefined = otherKeys.every(key => !checkedAnswers[key]);
  ...
  return (
    <ScrollView style={tw`flex bg-white`}>
        <Image style={tw`h-12 w-10 mx-auto mt-10 `} source={require('../assets/uvt.png')}/>
        <Text style={tw`text-black font-bold text-lg mt-10 p-3`}>{question.Category}</Text>
        <Text style={tw`text-black text-lg mx-auto p-3 pt-0`}>{index+1}. {question.QuestionTitle}</Text>
        {question.Image && <Image style={tw`h-50 w-80 mx-auto`} resizeMode='contain' source={image}/>}
        ...
        >
            <View style={tw`flex-1 items-center justify-center bg-black bg-opacity-50`}>
                <View style={tw`bg-white w-3/4 rounded-md p-4`}>
                    {valid?
                        <Text style={tw`text-black mx-auto text-2xl mb-20`}>Corect!</Text>
                    
                        : 
                        <View>
                            <Text style={tw`text-black mx-auto text-2xl mb-20`}>Incorect!</Text>
                            <Text style={tw`text-black mx-auto text-2xl mb-20`}>Raspunsuri corecte : {question.Answer && question.Answer.map((answer) => <Text> {answer} </Text>)} </Text>
                        </View>
                        
                    }
                    {valid && <TouchableOpacity style={tw`bg-blue-500 mx-auto w-1/2 rounded-md p-2 mt-2`} onPress={() => {setModalVisible(false); nextQuestion();}}>
                        <Text style={tw`text-white`}>Next</Text>
                    </TouchableOpacity>}
                    <TouchableOpacity style={tw`bg-gray-500 mx-auto w-1/2 text-md rounded-md p-2 mt-2`} onPress={() => setModalVisible(false)}>
            ...
            {answerKeys.map((answer) =>
                <View key={answer} style={tw`flex-1 flex-row p-6 pt-2`}>
                    <Checkbox
                        ...
                        onValueChange={(isChecked) => handleCheckboxChange(answer, isChecked)}
                        color={checkedAnswers[answerMapper[answer]] ? '#3B82F6' : '#3B82F6'}
                    />
                    <Text style={tw`my-auto text-md`}>{`${answerMapper[answer]}) ${question[answer]}`}</Text>
                </View>    
            )}
            ...
            {index < randomQuestions.length-1 && <TouchableOpacity style={tw`mx-auto bg-blue-500 flex w-3/4 rounded-md p-2 mt-10`} onPress={() => {nextQuestion()}}>
